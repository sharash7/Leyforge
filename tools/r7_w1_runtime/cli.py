"""Command line interface for W1 readiness, admission, and proof execution."""
...
from .execution import execute_w1, preflight_report
...
def _emit(value: object, output_format: str) -> None:
    if output_format == "json":
        ...
        print(f"Leyforge R7 W1 {value.get('status', 'UNKNOWN')}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Leyforge governed R7 W1 owner/spatial proof runner")
    ...
    execute.add_argument("--state-path", type=Path, default=Path("docs/rebuild/r7/w1-execution-state.json"))
...
def main() -> int:
    ...
    try:
        if args.command == "preflight":
            ...
            result = execute_w1(args.source_revision, args.run_root, args.retained_root, args.state_path, args.actual_execution_authorized)
