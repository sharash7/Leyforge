"""Clean W1 provider-probe export and real process execution."""
...
PROBE_SOURCE = ROOT / "proofs/r7/w1/provider_probe"
W1_SOURCE = ROOT / "proofs/r7/w1"
...
def _export_preset(role: str, template_path: Path) -> str:
    name = "W1 Client Probe" if role == "client" else "W1 Dedicated Probe"
...
custom_features="r7_w1_proof,{feature}"
...
def export_one(source_revision: str, role: str, run_root: Path, execution: ProofExecution) -> ExportedBuild:
    if role not in {"client", "headless"}:
        raise ValueError("unsupported W1 export role")
    lock = load_lock()
    ...
    try:
        ...
        raise ValueError("W1 run root must remain under repository .local") from exc
    if build_root.exists():
        raise ValueError(f"clean W1 build root already exists: {build_root}")
    ...
    preset = "W1 Client Probe" if role == "client" else "W1 Dedicated Probe"
    artifact_path = output_root / f"leyforge-w1-{role}-probe.exe"
    ...
    build = BuildManifest(
        ...
        role=role,
        profile="release-like-w1-provider-proof",
        precision="single-precision-provider-with-canonical-revision-guards",
        platform="windows-x86_64",
        content_identity=_tree_identity(W1_SOURCE),
        schema_identity="leyforge-r7-w1-owner-spatial-runtime-v1",
        clean_export=True,
    ...
    if issues:
        raise ValueError("W1 build manifest is invalid: " + "; ".join(issues))
    ...
    if export_result.process.exit_code != 0 or export_result.process.timed_out or not artifact_path.is_file():
        raise RuntimeError("W1 Godot export failed: " + json.dumps(export_result.to_dict(), ensure_ascii=True))
    ...
    lane = SmokeLane(lane_id=f"W1-{role.upper()}-EXPORT-SMOKE", proof_id=execution.proof_id, role=role, build=build, artifact=artifact, argv=tuple(argv))
    smoke = lane.execute(controller, timeout_seconds=60.0, proof_execution=execution)
    report = _parse_prefixed_json(smoke.process.stdout, "LEYFORGE_W1_SELF_REPORT ")
    if report.get("status") != "PASS" or report.get("build_identity") != build.build_identity or report.get("role") != role:
        raise RuntimeError("W1 runtime self-report did not match its build")
    if not smoke.evidence_eligible:
        raise RuntimeError("W1 export smoke is not evidence eligible")
    files = [
    ...
    content = {
        "schema_version": "prd07-w1-role-content-manifest-v1",
        "role": role,
...
def run_provider_probe(exported: ExportedBuild, execution: ProofExecution, iterations: int = 256) -> Dict[str, Any]:
    ...
    if exported.build.role == "headless":
        argv.append("--headless")
    argv.extend(["--", "--mode", "provider", "--proof-id", execution.proof_id, "--run-id", str(execution.run_id), "--iterations", str(iterations)])
    lane = SmokeLane(
        lane_id=f"{execution.proof_id}-W1-PROVIDER",
    ...
    process = lane.execute(controller, timeout_seconds=90.0, proof_execution=execution)
    report = _parse_prefixed_json(process.process.stdout, "LEYFORGE_W1_PROVIDER_REPORT ")
    if report.get("proof_id") != execution.proof_id or report.get("run_id") != execution.run_id:
        raise RuntimeError("W1 provider report execution identity differs")
    if report.get("build_identity") != exported.build.build_identity:
        raise RuntimeError("W1 provider report build identity differs")
