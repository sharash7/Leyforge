"""Git-clean admission for the bounded W2 proof and evidence surface."""
...
MANIFEST_PATH = ROOT / "docs/rebuild/r7/w2-execution-boundary.json"
FIXED_FILES = ("tools/tests/test_r7_w2_runtime.py", "docs/rebuild/r7/w2-readiness.json")
FIXED_TREES = ("proofs/r7/w2", "tools/r7_w2_runtime")
OPTIONAL_FILES = ("docs/rebuild/r7/w2-execution-state.json", "docs/rebuild/r7/w2-execution-completion-receipt.json")
...
def build_manifest(implementation_commit: str) -> Dict[str, Any]:
    ...
    if result.returncode:
        raise RuntimeError(result.stderr.strip() or "W2 Git-clean hashing failed")
    blobs = result.stdout.splitlines()
    if len(blobs) != len(paths):
        raise RuntimeError("W2 Git-clean hashing returned a different path count")
    artifacts = []
    ...
    state_path = ROOT / "docs/rebuild/r7/w2-execution-state.json"
