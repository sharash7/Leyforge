"""Guarded W2 execution and append-only evidence retention."""
...
from proofs.r7.w2.runtime.runner import run_proof
...
def source_execution_issues(source_revision: str) -> Tuple[str, ...]:
    ...
    governed = (
        "proofs/r7/w2", "tools/r7_w2_runtime", "tools/tests/test_r7_w2_runtime.py",
        "docs/rebuild/r7/w2-readiness.json", "docs/rebuild/r7/w2-execution-boundary.json",
        "tools/verify.py", "tools/verify_rebuild_boundary.py",
    )
    if _git("status", "--porcelain", "--", *governed):
        issues.append("governed W2 proof source has uncommitted changes")
    return tuple(issues)
...
def preflight_report(implementation_commit: str, check_local: bool) -> Dict[str, Any]:
    ...
    return {
        "schema_version": "leyforge-r7-w2-preflight-v1",
...
def _artifact_summary(exported: ExportedBuild) -> Dict[str, Any]:
    ...
    artifact["retention"] = {
        ...
        "reproducer": "python -m tools.r7_w2_runtime execute --actual-execution-authorized ...",
    }
    return artifact
...
def _retain(
    ...
    evidence_file = EvidenceFile.from_path(evidence_id, "observed-w2-proof-result", raw_source)
    pack = EvidencePack(
    ...
    if issues:
        raise ValueError("W2 evidence pack failed validation: " + "; ".join(issues))
    local_pack = pack.retain(run_root / "validated-evidence")
    portable = retained_root / str(execution.run_id)
    if portable.exists():
        raise ValueError(f"append-only W2 evidence destination exists: {portable}")
...
def _write_state(path: Path, source_revision: str, rows: list[Dict[str, Any]], builds: Mapping[str, ExportedBuild]) -> None:
    ...
    value = {
        "schema_version": "prd07-w2-execution-state-v1",
        "package": "R7-W2-NETWORK-PERSISTENCE-RECOVERY-MIGRATION-READINESS",
        "package_state": "EXECUTING" if pending else "W2-EXECUTION-COMPLETE",
        "source_revision": source_revision,
        "gameplay_permission": "CLOSED",
        "prd08_evaluation": "CLOSED",
        "w4_fcc13e_revalidation": "REQUIRED",
        "prior_readiness_source": "docs/rebuild/r7/w2-readiness.json",
...
def execute_w2(
    ...
    if not actual_execution_authorized:
        raise ValueError("actual W2 proof execution requires the explicit authorization flag")
    source_issues = source_execution_issues(source_revision)
    if source_issues:
        raise ValueError("W2 source is not execution ready: " + "; ".join(source_issues))
    ...
    if tracked_readiness.get("status") != "PASS" or any(row.get("state") != "READY" for row in tracked_readiness.get("proofs", [])):
        raise ValueError("tracked W2 readiness does not make all twenty proofs individually READY")
    preflight = preflight_report(str(tracked_readiness["implementation_commit"]), check_local=True)
    if preflight["status"] != "PASS":
        raise ValueError("W2 preflight failed: " + "; ".join(preflight["issues"]))
    run_root = run_root.resolve()
    retained_root = retained_root.resolve()
    state_path = state_path.resolve()
    if run_root.exists():
        raise ValueError("W2 run root must be absent for append-only execution")
    ...
    return {
        "schema_version": "prd07-w2-execution-result-v1",
        "status": "COMPLETE",
        "package": "R7-W2-NETWORK-PERSISTENCE-RECOVERY-MIGRATION-READINESS",
        "source_revision": source_revision,
