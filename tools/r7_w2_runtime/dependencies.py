"""W2 dependency reference validation without duplicating W0 authority."""
...
REFERENCE_PATH = ROOT / "proofs/r7/w2/dependency-reference.json"


def load_reference() -> Dict[str, Any]:
    value = json.loads(REFERENCE_PATH.read_text(encoding="utf-8-sig"))
    if not isinstance(value, dict):
        raise ValueError("W2 dependency reference must be an object")
    return value
...
def reference_issues() -> Tuple[str, ...]:
    ...
    issues = []
    if reference.get("schema_version") != "leyforge-r7-w2-dependency-reference-v1":
        issues.append("unsupported W2 dependency-reference schema")
    if reference.get("gameplay_permission") != "CLOSED":
        issues.append("W2 dependency reference cannot open gameplay")
    if reference.get("production_dependency_state") != "UNCHANGED-PLANNED-UNINSTALLED":
        issues.append("W2 proof reference changed production dependency state")
    ...
    if lock_path != ROOT / "proofs/r7/w0/dependencies.lock.json" or not lock_path.is_file():
        issues.append("W2 must reference the governed W0 dependency lock")
        return tuple(issues)
    if hashlib.sha256(lock_path.read_bytes()).hexdigest() != reference.get("source_lock_sha256"):
        issues.append("W0 dependency lock SHA-256 differs from W2 reference")
    ...
    if result.returncode or result.stdout.strip() != reference.get("source_lock_git_blob"):
        issues.append("W0 dependency lock Git blob differs from W2 reference")
    lock = load_lock()
    ...
    for name, revision in reference.get("component_revisions", {}).items():
        if str(lock.get("components", {}).get(name, {}).get("revision")) != str(revision):
            issues.append(f"W2 dependency revision differs for {name}")
    if reference.get("local_patch_status") != lock.get("local_patch_status"):
        issues.append("W2 local patch status differs from the governed lock")
